...
async fn perf_minimal(State(pool): State<Pool>) -> Json<Vec<MinimalResult>> {
    let client = pool.get().await.unwrap();
...
async fn perf_post(State(pool): State<Pool>, Json(body): Json<PostBody>) -> Json<Vec<PostResult>> {
    let client = pool.get().await.unwrap();
...
async fn perf_nested(State(pool): State<Pool>, Query(query): Query<NestedParams>) -> Json<Vec<NestedResult>> {
    let client = pool.get().await.unwrap();
...
async fn perf_large_payload(State(pool): State<Pool>, Query(query): Query<LargePayloadParams>) -> Json<Vec<LargePayloadResult>> {
    let client = pool.get().await.unwrap();
...
async fn perf_many_params(State(pool): State<Pool>, Query(query): Query<ManyParams>) -> Json<Vec<ManyParamsResult>> {
    let client = pool.get().await.unwrap();
