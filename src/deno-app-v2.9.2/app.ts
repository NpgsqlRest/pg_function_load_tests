...
// matching natively multi-threaded runtimes (Go, Rust, .NET, JVM). The primary
// process spawns WORKERS children that share port 3105 via SO_REUSEPORT.
const WORKERS = parseInt(Deno.env.get('WORKERS') || '') || navigator.hardwareConcurrency;
...
// Pool is per worker process; workers x max ~= 100 aggregate
// to match the single-pool services.
const sql = postgres({
    host: Deno.env.get('DB_HOST') || 'postgres',
    port: 5432,
    database: Deno.env.get('DB_NAME') || 'testdb',
    username: Deno.env.get('DB_USER') || 'testuser',
    password: Deno.env.get('DB_PASSWORD') || 'testpass',
...
Deno.serve({
    ...
    hostname: '0.0.0.0',
    reusePort: true,
    onListen: ({ port }) => console.log(`Deno server running on port ${port}`),
}, async (request) => {
